...
from utilitaries import get_random_pattern, mask_match, printl
import random
...
class Condition():
    ...
    In our implementation of phonetic change, we distinguished 2 different subclasses of conditions :
...
class P_condition(Condition) :
    ...
    def encode_P_cond(self) :
        """
        Encodes the change for our machine friendly log
        """
    ...
    def decode_P_cond(string) :
        """
        Decodes the change for our machine friendly log 
        """    
        string = string [8:]
        string = string.split(" | ")
...
class S_condition (Condition) : 
    """
    A class to represent a condition concerning the Syllabic structure of the word
    ...
    ...
    def test(self, word, rank, verbose = False) :
        ...
        if self.abs_position != 42 :
            return self.match_syl(word.syllables[self.abs_position] )
        
        
        # if the position is relative : index of the syllable we need to study
        index  = rank + self.rel_pos
        if abs(index ) not in range(nb_syl) : return False

        return self.match_syl(word.syllables[index])
        
    
    
        
     
    def match_syl(self, syl) :
        """
        States if a given syllable respects the pattern of a condition
        """
        
        return self.tone == syl.tone and self.stress == syl.stress and self.length == syl.length
